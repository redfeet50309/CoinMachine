// Main Alpine app. Wires watchlist + data fetching + chart toggling.

import {
  fetchWatchlist, fetchIndex, fetchMeta, fetchStock,
  getPat, writeWatchlist, readLocalWatchlist, writeLocalWatchlist,
} from './api.js';
import {
  trendClass, zoneClass, histClass, formatPrice, formatNum, formatTime,
} from './rules.js';
import { drawCharts, destroyCharts } from './chart.js';
...
window.app = () => ({
  ...
  async refresh() {
    // Server-side watchlist wins; fall back to local copy (offline / no-pat path)
    let wl;
    try {
